...
module.exports = async () => {
    try {
        const options = {
            scriptPath: `${process.cwd()}/../web-scraping/`,
            args: [process.env.INSTA_EMAIL, process.env.INSTA_PW]
        };
        PythonShell.run('instagram-scraping.py', options, async (err, results) => {
            ...
            console.log('핫플레이스 업데이트');

            const rawPlaces = results;
